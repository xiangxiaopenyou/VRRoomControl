...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.tableView) {
        ...
        cell.doctorLabel.text = [NSString stringWithFormat:@"医生：%@", model.doctorName];
        cell.typeLabel.text = [model.source integerValue] == 1 ? @"类型：线上" : @"类型：线下";
        cell.diseaseLabel.text = [NSString stringWithFormat:@"病症：%@", model.disease];
        cell.dateLabel.text = [NSString stringWithFormat:@"日期：%@", model.createdAt];
